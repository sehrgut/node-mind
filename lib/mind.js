...
/*
	Messy EventEmitter inheritance, but it's better than delegation. Idea from:
	http://www.mshiltonj.com/blog/2011/10/04/nodejs-eventemitter-example-with-custom-events/
	
	Definitely can't use normal pattern of prototype=new EventEmitter(),
	since we need all db objects to have independent event queues.
*/
...
MindManager.prototype = Object.create(EventEmitter.prototype);
...
MindManager.prototype._catchSignals = function () {
	...
	process.on('exit', this._sigcatcher)
...
MindManager.prototype._uncatchSignals = function () {
	if(this._sigcatcher) {
		process.removeListener('exit', this._sigcatcher);
